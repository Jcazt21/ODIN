...
| 7 | 🟡 **Golden set** (150-300 artículos) + `scripts/evaluate.py` — herramienta y formato listos y probados (`tests/eval/`), ampliado a **42** artículos reales repartidos entre 6 fuentes (etiquetado asistido, no revisado por un humano aparte). Faltan ~108-258 artículos más — ver `tests/eval/README.md` | Sin esto no sabes si mejoras; hay números publicados sin respaldo |
...
| # | Tarea |
|---|---|
| 10 | ✅ ~~Linaje: `analyzer_name/model/version`, `analyzed_at`, `schema_version` (§2.1)~~ — implementado (pendiente de commit): columnas en `Article` ([db/models.py](db/models.py)) + migración [alembic/versions/73ed4b404bce_articles_linaje_del_analisis.py](alembic/versions/73ed4b404bce_articles_linaje_del_analisis.py); `Analyzer` expone `name/model/version` ([analysis/base.py](analysis/base.py), implementado en `LocalAnalyzer` y `GeminiAnalyzer`); los tres puntos de escritura (`api.py::analyze`, `api.py::save_article`, `pipeline.py::_persist`) lo estampan; expuesto en `ArticleDetail`; tests en `tests/test_pipeline.py::TestPersistRecordsLineage` y `tests/test_api_canonical_entities.py::test_stamps_analyzer_lineage` |
...
| 15 | ✅ ~~Cola de trabajos: `/api/analyze` → `202` + `job_id` + worker (§3.1)~~ — hecho 2026-08-04: nueva tabla `analyze_jobs` ([db/models.py](db/models.py) + migración `eb1a1e0e15a2`), clave pública `job_id` en UUID (no el id autoincremental, para no dejar adivinar el volumen de análisis). `POST /api/analyze` en [api.py](api.py) sigue devolviendo `200` directo si la URL ya está guardada (nada que encolar); si es nueva, crea la fila `pending` y devuelve `202` + `job_id` de inmediato — el trabajo pesado (`_fetch_and_extract` + NLP + arbitraje opcional de Gemini + canonicalización, hasta ~60s) corre en `BackgroundTasks` (`_run_analyze_job`), fuera del ciclo request/response. Cualquier excepción ahí se captura y queda en `error`/`status=failed` del job, nunca se propaga sin destino. `GET /api/jobs/{job_id}` (autenticado, `404` si no existe) devuelve `status`/`error`/`result`. Tipos TS regenerados (`npm run generate:types`) y frontend actualizado: `analyzeUrl()` en [frontend/src/lib/odin-api.ts](frontend/src/lib/odin-api.ts) hace el polling internamente (intervalo 1.5s, timeout 120s) y sigue resolviendo en `ArticleDetail`, así que el resto del código no cambia; ahora acepta un callback `onStatus` que [App.tsx](frontend/src/App.tsx) usa para distinguir "encolando" de "extrayendo y analizando" en la UI. Worker: para el volumen actual (uso manual, no batch) `BackgroundTasks` es la opción barata que pide la tarea — no se introdujo Celery/RQ/arq sin evidencia de que el threadpool de `BackgroundTasks` sea insuficiente. Tests nuevos en `tests/test_api_analyze_jobs.py` (URL ya guardada sin crear job, `202`+job_id, job completado con resultado, job fallido reportado sin excepción sin manejar, URL inválida no crea job, `404` de job inexistente, auth requerida), sin red ni spaCy/Gemini real. Verificado con suite completa, build de frontend y prueba manual end-to-end con `TestClient` |
| 16 | ✅ ~~Throttle por dominio + `robots.txt` (§2.6)~~ — hecho 2026-08-04: `_DomainThrottle` en [scrapers/base.py](scrapers/base.py) garantiza `REQUEST_DELAY` segundos entre peticiones exitosas al mismo host (antes solo gobernaba el backoff de reintentos), con un lock que serializa "esperar el turno" pero no la descarga en sí, así que `FETCH_WORKERS` sigue paralelizando entre dominios distintos. `_RobotsCache` lee y cachea `robots.txt` por dominio (`urllib.robotparser`); un dominio con robots.txt inalcanzable se trata como "todo permitido" (comportamiento estándar del protocolo, no falla cerrado). `BaseScraper.fetch()` consulta ambos antes de cada intento: si la ruta está deshabilitada no pide nada, y si el sitio declara `Crawl-delay` mayor que `REQUEST_DELAY` usa ese valor. Opt-out solo para pruebas locales vía `ODIN_RESPECT_ROBOTS_TXT=0` ([config.py](config.py)). [README.md#cortesía--legalidad](README.md) actualizado para describir el comportamiento real (ya no "nada de esto está implementado"). Tests nuevos en `tests/test_scrapers_base.py` (`TestDomainThrottle`, `TestRobotsCache`, `TestFetchRespectsRobotsAndThrottle`), sin red real |
| 17 | ✅ ~~Normalización de fechas a UTC aware (§2.7)~~ — hecho 2026-08-04: `_parse_date` ([scrapers/base.py](scrapers/base.py)) ya no devuelve naive por la rama `strptime`/fecha-sola; toda fecha sin offset se asume en hora de Santo Domingo (UTC-4 fijo, sin horario de verano) y se convierte a UTC aware antes de devolverse — mismo tratamiento para ISO con offset/`Z`. Como los tres puntos de escritura (`scrapers/base.py::extract`, `api.py::analyze`, `api.py::save_article`) pasan por esta única función, `Article.published_at` siempre llega aware a la columna `DateTime(timezone=True)`, igual que `scraped_at`/`created_at`/`updated_at` (ya usaban `_utcnow()` aware). Los filtros `date_from`/`date_to` de `GET /api/articles` ([api.py](api.py)) no necesitaron cambios: al recibir fechas sueltas se interpretan ahora con la misma convención (medianoche en Santo Domingo), consistente con lo persistido. Tests actualizados en `tests/test_scrapers_base.py::TestParseDate` (todas las ramas devuelven aware) y `tests/test_api_filters.py::TestDateRangeFilter` (datos de prueba en UTC aware, verificando el borde de día en la zona horaria correcta) |
| 18 | ✅ ~~Logs estructurados + `/metrics` + tabla `crawl_runs` + Sentry (§7.1)~~ — hecho 2026-08-04: `structlog` (JSON en prod / consola en dev, correlation ID por request/corrida vía `ContextVar`) en [observability.py](observability.py), usado por `main.py` y `api.py`. `GET /metrics` (`prometheus-client`, registro propio) con métricas de pipeline (artículos por fuente/etapa, duración y resultado de corrida), latencia/error por endpoint (middleware nuevo) y llamadas+tokens de Gemini (instrumentado en `analysis/gemini_analyzer.py`, sin llamar la API real). Tabla `crawl_runs` ([db/models.py](db/models.py) + migración `16b256a1a0ab`) persiste el resumen que antes solo se imprimía por consola; `pipeline.run()` la escribe al empezar/terminar cada corrida y `GET /api/crawl-runs` la expone. Sentry opt-in vía `ODIN_SENTRY_DSN` (`init_sentry()`, sin DSN no hace ninguna llamada de red). Verificado con la suite de tests completa (sin regresiones) y pruebas manuales end-to-end (pipeline con SQLite, `/metrics`, `/api/health`) |
| 19 | ✅ ~~CI en GitHub Actions: lint + types + tests + `pip-audit`. Incluye instalar desde `requirements.lock`~~ — hecho 2026-08-04 (pendiente de commit): [.github/workflows/ci.yml](.github/workflows/ci.yml) con cuatro jobs (`lint`/`ruff`, `types`/`mypy`, `test`/`pytest`, `pip-audit`). `Dockerfile.backend` instala ahora desde el lock (`COPY requirements.lock .` + `pip install --require-hashes`) en vez de `requirements.txt` — los builds pasan a ser reproducibles. Subió la imagen base a `python:3.13-slim` (el lock se genera con `--python-version 3.13`, hashes no cubren cp312). Verificado localmente: `ruff`/`mypy`/`pytest` corridos en contenedor `python:3.13-slim` (mismo runtime que Actions) y `docker build -f Dockerfile.backend .` completo con éxito (imagen arranca, importa `api.py`). En el camino salieron dos bugs preexistentes que **task 19 no arregla, solo destapa**: (1) `requirements.lock` estaba desactualizado — le faltaba `groq` (agregado a `requirements.txt` sin regenerar el lock) — y al regenerarlo apuntando a Linux real (antes se había generado en una plataforma no-Linux) `torch` resolvía a la build CUDA (+nvidia-\*/triton, ~4.5GB); se resolvió fijando `--torch-backend cpu --python-platform x86_64-unknown-linux-gnu` al compilar (ver README.md#requirementslock) — `Dockerfile.backend` ya no necesita el preinstall manual de torch que tenía. Los jobs `types`/`test` de CI usan [requirements-ci.txt](requirements-ci.txt) (`pip install -c requirements.lock`) en vez del lock completo, para no bajar pysentimiento/spacy/torch en cada corrida. (2) mypy y pytest, corridos en verde contra este setup, revelan errores reales en el trabajo en progreso de linaje de análisis (§10, pendiente de commit): `pipeline.py::_persist` referencia una variable `analyzer` que no existe en su firma (`NameError`/`F821`/mypy `name-defined`), y 11 tests fallan porque `api.py::_accent_insensitive_contains` usa el operador `~*` de Postgres (búsqueda insensible a acentos) contra SQLite en memoria, que no lo soporta. `docs/docker.md` y `README.md#requirementslock` actualizados en consecuencia |
| 20 | Rate limiting + presupuesto de Gemini con cortacircuito (§5.3) |
| 21 | ✅ ~~Endpoints de borrado/rectificación de artículos y entidades (§8.2)~~ — hecho 2026-08-03: `PUT`/`DELETE /api/articles/{id}` y `PUT`/`DELETE /api/entities/{id}` en [api.py:517](api.py#L517), con tests en `tests/test_article_entity_rectification.py`. UI de rectificación de **artículos** implementada (pendiente de commit) en [frontend/src/components/ReportsList.tsx](frontend/src/components/ReportsList.tsx) — formulario `ArticleEditForm` (editar tema, sentimiento, encuadre, actores) + borrado con confirmación, llamando a `updateArticle`/`deleteArticle` de [odin-api.ts](frontend/src/lib/odin-api.ts). **Sigue pendiente**: `updateEntity`/`deleteEntity` (mismo archivo) están definidas pero no se llaman desde ningún componente — la rectificación/borrado de una entidad individual no tiene UI. Tampoco base legal/retención ni `docs/LEGAL.md` |
...
| 24 | ✅ ~~Partir `api.py` en routers + capa de servicios (§9.2)~~ — hecho 2026-08-05: `api.py` (1716 líneas) pasó a paquete `api/` (`uvicorn api:app` sigue funcionando igual: `api/__init__.py` expone `app`) con `api/schemas.py` (todos los `BaseModel` de request/respuesta), `api/deps.py` (`get_session` reexportado desde `db.session` — único punto de import que los tests parchean) y `api/routers/{analyze,articles,entities,aliases,canonical_entities,scrape_jobs,misc}.py`, un módulo por grupo de rutas. La lógica de negocio (queries SQLAlchemy, antes mezclada con los handlers HTTP) se movió a `services/{analyze_service,article_service,entity_service,alias_service,canonical_entity_service,scrape_job_service,analyzer_registry}.py`: los routers ahora solo traducen request/response, no hablan SQLAlchemy directo. `tests/conftest.py` (fixture `api_client`) pasó de parchear `api_module.get_session` a `api.deps.get_session` — todos los servicios llaman `deps.get_session()` por atributo de módulo (no `from api.deps import get_session`, que copiaría el nombre y el monkeypatch dejaría de alcanzarlos). `pyproject.toml` actualizado (`tool.setuptools.packages.find` incluye `api*`/`services*`; `tool.mypy.files` apunta a los paquetes en vez de `api.py`). Verificado con la suite completa (191 tests, 184 passed + 7 skipped preexistentes, mismo conteo que antes del split), `ruff check` en verde, y `scripts/generate_openapi.py` produciendo el mismo schema (paths idénticos, incluidos los de `/api/scrape-jobs` que el `openapi.json` committeado tenía desactualizado de antes). |
...
| 26 | ✅ ~~Frontend: React Query, rutas, error boundary, Vitest, partir componentes grandes~~ — hecho 2026-08-05: `react-router-dom` reemplaza la navegación por `?tab=` de `App.tsx` — rutas reales `/analyze`, `/reports`, `/reports/:id`, `/entities`, `/aliases`, `/scrape`, `/login`, con `ProtectedRoute` y `Layout` (antes `Workspace`) montando un `<Outlet/>`. `@tanstack/react-query` reemplaza los `useEffect`+`useState` de carga a mano en `ReportsList`, `CanonicalEntityManager`, `AliasManager` y el flujo de análisis/guardado — hooks por dominio en `lib/queries/{articles,canonical-entities,aliases,auth}.ts` con query keys, invalidación en cada mutación y `keepPreviousData` en listados paginados; `ScrapeTab` mantiene su polling manual a propósito (job de larga duración con cancelación cooperativa, no encaja en el modelo query/mutation). `ErrorBoundary` (clase, con `getDerivedStateFromProps` para resetear al cambiar de ruta) envuelve el `<Outlet/>` en `Layout.tsx`. Vitest + Testing Library configurados (`vitest.config.ts`, jsdom, `src/test/setup.ts`) con 17 casos en 4 archivos: `format.test.ts`, `ErrorBoundary.test.tsx`, `SentimentBadge.test.tsx` y una prueba de integración de `AliasManager` con React Query real y `odin-api` mockeado (carga, vacío, creación). `App.tsx` bajó de 452 a 111 líneas; `ReportsList.tsx` (724 líneas) se partió en `pages/{ReportsPage,ReportDetailPage}.tsx` + `components/reports/{FilterBar,ReportsTable}.tsx`, cada uno bajo 230 líneas. Verificado con `tsc -b --force` limpio, `oxlint` sin advertencias nuevas, `vitest run` en verde y `vite build` exitoso; rutas profundas (`/reports/1`, `/login`) confirmadas con 200 en el fallback SPA de Vite dev, igual que `try_files` de `nginx.conf` en producción |
| 27 | ✅ ~~Hardening de contenedores: `USER` no-root, healthchecks, cabeceras en nginx (§5.5)~~ — hecho 2026-08-05: `Dockerfile.backend` ya corría como `appuser` no-root (de un cambio anterior) — sin trabajo pendiente ahí salvo dejar explícito en comentario por qué **no** lleva `HEALTHCHECK` propio (la misma imagen sirve `backend` con uvicorn y `scraper` con `main.py` como CLI de una sola corrida; un test fijo a `/api/health` marcaría al scraper como "unhealthy" para siempre). `frontend/Dockerfile` pasa a `USER nginx` con `listen 8080` (solo root puede bindear <1024) y `chown` de `/usr/share/nginx/html`, `/var/cache/nginx`, `/var/log/nginx`, `/etc/nginx/conf.d` y el pid file; `docker-compose.yml` actualiza el mapeo a `3000:8080`. `frontend/nginx.conf` suma `server_tokens off`, `location = /healthz` (liveness propio, no depende del backend) y cabeceras de seguridad (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, CSP con `style-src 'unsafe-inline'` porque la UI usa `style={{...}}` inline extensivamente) — factorizadas en `frontend/security-headers.conf` e incluidas explícitamente en `/assets/` y `/index.html` porque nginx **no hereda** `add_header` del `server{}` hacia un `location` que ya define el suyo propio (Cache-Control ahí), gotcha real encontrado al verificar con `curl -sD -` que las cabeceras faltaban justo en esos dos locations antes del fix. `docker-compose.yml` suma `deploy.resources.limits` (cpus/memory) en los 4 servicios y `healthcheck` al servicio `frontend` (antes solo `db`/`backend` lo tenían). Verificado con `docker build` + `docker run` real (no solo lectura de config): `whoami` dentro del contenedor devuelve `nginx`; las cabeceras aparecen en `/` y `/assets/`; el HEALTHCHECK inicial fallaba con "connection refused" porque `localhost` resuelve a `::1` dentro de la imagen base de nginx y el server solo escucha en `0.0.0.0:8080` — corregido a `127.0.0.1` en el `HEALTHCHECK` del Dockerfile y en el healthcheck de compose, tras lo cual `docker inspect` reporta `healthy`; `docker compose config` confirma que `deploy.resources.limits` se resuelve sin swarm (Compose v2 lo aplica directo) |
| 28 | Backups automáticos + procedimiento de restauración probado (§7.5) |
| 29 | ✅ ~~Documentación faltante: ARQUITECTURA, ADRs, DATA_DICTIONARY, RUNBOOK, LEGAL, PRECISION, LICENSE (§10.2)~~ — hecho 2026-08-05: [`docs/ARQUITECTURA.md`](docs/ARQUITECTURA.md) (estilo C4: contexto, contenedores, componentes de `api/`, `services/`, `analysis/`, `scrapers/`, `db/`, frontend, Docker), [`docs/adr/`](docs/adr/) con 5 ADRs (trafilatura+sitemaps, Alembic, `thinking_budget=0`, `_predict_batch` en lotes de 32, selección explícita de analizador), [`docs/DATA_DICTIONARY.md`](docs/DATA_DICTIONARY.md) (las 7 tablas de `db/models.py` columna por columna, incluidas `analyze_jobs`/`scrape_jobs`/`crawl_runs`/`canonical_entities`/`entity_aliases` que no estaban en el diagnóstico original de §10.2), [`docs/RUNBOOK.md`](docs/RUNBOOK.md) (health check, variables de entorno, fuente caída, reintentar/cancelar jobs, rotar llaves, migraciones Alembic, backup/restore manual — señala que no hay tooling de backup propio, gap real), [`docs/LEGAL.md`](docs/LEGAL.md) (tabla de ToS por medio marcada 🔴 pendiente de revisión humana, checklist Ley 172-13/GDPR, estado real de endpoints de rectificación/borrado), [`docs/PRECISION.md`](docs/PRECISION.md) (metodología de `scripts/evaluate.py` + golden set; **estado real: 7 artículos, insuficiente para publicar cifras** — reemplaza los rangos sin evidencia del README, no los repite), y `LICENSE` propietario/todos los derechos reservados (decisión del dueño: Odin se entrega/vende a cliente, no es open source). Los hechos de cada doc se verificaron contra el código actual (`api/`, `services/`, `alembic/versions/`), no contra este diagnóstico de §10.2, que ya estaba desactualizado en varios puntos (p. ej. `api.py` ya no existe como archivo único, ver ítem 24). |
| 30 | ✅ ~~Actualizar README y PROCESOS al estado real; checkpoint de docs en el flujo de cambios (§10.1)~~ — hecho 2026-08-06: README.md corregido en drift real (Python 3.13 no 3.12 en Docker, `api/`+`services/` no `api.py`, 9 fuentes no 8, 5 motores de analizador no 2 — local/gemini/groq/hybrid/groq+gemini —, `ODIN_GEMINI_ARBITER` corregido: no es exclusivo de `local`, 5 jobs de CI no 4, tabla de precisión reemplazada por enlace a `docs/PRECISION.md` en vez de porcentajes sin evidencia); nueva sección "Documentación" indexando los 9 docs de `docs/`; `docs/PROCESOS.md` reescrito de punta a punta (antes describía 2 fuentes y 2 analizadores de un flujo que ya no es el principal — no cubría el flujo a demanda en absoluto, que era precisamente el hallazgo de §10.1) con el flujo a demanda como sección 2 y el masivo como proceso secundario, los 5 analizadores con sus ADRs, y el modelo de datos actual (`CanonicalEntity`, `analyze_jobs`, `scrape_jobs`, `crawl_runs`). `docs/docker.md` corregido en los mismos puntos de drift (puertos reales `5433:5432`/`3000:8080`, Python 3.13, `api/` no `api.py`). Checkpoint de documentación: no solo prosa — `scripts/check_docs_freshness.py` corre en cada commit (`.pre-commit-config.yaml`) y avisa (sin bloquear) cuando el commit toca `db/models.py`/`api/schemas.py`/`api/routers/`/`scrapers/`/`analysis/base.py`/`config.py`/`alembic/versions/` sin tocar ningún archivo de `docs/` o el README en el mismo commit; verificado disparando y no disparando según corresponda. |
...
- [x] Linaje del análisis persistido (analizador, modelo, versión, fecha) — implementado, pendiente de commit (#10)
