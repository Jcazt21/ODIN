...
```bash
odin --list-sources          # ver los periódicos disponibles
odin --init-db               # crear las tablas y salir
odin                         # rastrear TODOS los periódicos
odin --source diario_libre   # rastrear solo uno
odin --source listin_diario --source diario_libre   # varios
odin --limit 10              # máximo 10 artículos por periódico
odin --analyzer local        # motor de análisis local (por defecto)
odin --analyzer gemini       # motor Google Gemini (ver sección 7)
```

Se pueden combinar, p.ej.:
```bash
odin --source diario_libre --limit 20
```
...
### Con `odin.core.report` (rápido, en consola)
```bash
python -m odin.core.report                       # resumen: fuentes, sentimiento, más mencionados
python -m odin.core.report --entity "Abinader"   # todas las menciones de una figura/empresa
```
...
```cron
0 7 * * * cd /Users/jazar/Documents/Projects/Odin && ./.venv/bin/odin >> odin.log 2>&1
```

**Windows (Programador de tareas)**: crea una tarea básica con
"Iniciar un programa" apuntando a `.venv\Scripts\odin.exe`, sin argumentos,
y "Iniciar en" la carpeta del proyecto — desde el Programador de
